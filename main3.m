...
%----------增加距离判别功能，放置有些距离数据为0，导致算法出错-----2019.12.31，晚
%%
clc
close all
instrfindall
...
%%
s=serial('COM17','BaudRate',9600,'Parity','none','DataBits',8,'StopBits',1)% 串口设置
...
num=1;  %初始循环的次数为0
[Ajunzhi,Bjunzhi,Cjunzhi,Djunzhi]=deal([]);  %连续赋值，创建数组
[Ajunzhi(200),Bjunzhi(200),Cjunzhi(200),Djunzhi(200)]=deal(1);  %数组大小为200，第200位为1
[Ajunzhi(num),Bjunzhi(num),Cjunzhi(num),Djunzhi(num)]=deal(333);  %数组第一位为333
...
%%
    ...
    TAG=[830,774];  %待定位点的位置100
    H=160; %带定位点的高度H，单位cm78
    h=jizhanZ(1,1)-H;
    num=num+1,  %记录循环的次数
%%
    %读串口数据，存txt，并提取数据
...
%     Ajunzhi=0.993*Ajunzhi-17.2,  %自身误差校正后的数据unzhi-17.2,
%     Cjunzhi=0.993*Cjunzhi-17.2,
%     Djunzhi=0.993*Djunzhi-17.2,
    [Ajunzhi(num),Bjunzhi(num),Cjunzhi(num),Djunzhi(num)]=deal(Ajunzhi,Bjunzhi,Cjunzhi,Djunzhi);  %数组第一位为333
    diedai(Ajunzhi,num)   %返回Ajunzhi(num)的值
    diedai(Bjunzhi,num)
    diedai(Cjunzhi,num)
    diedai(Djunzhi,num) 
    %------------------------------------------%
    ...
    DX=D(1,1);DY=D(1,2);
    %----------未知量满足的等式关系--------------%
    eq1=(x-AX)^2 + (y-AY)^2 -Ajunzhi(num)^2-h^2;   
    eq2=(x-BX)^2 + (y-BY)^2 -Bjunzhi(num)^2-h^2;  
    eq3=(x-CX)^2 + (y-CY)^2 -Cjunzhi(num)^2-h^2;  
    eq4=(x-DX)^2 + (y-DY)^2 -Djunzhi(num)^2-h^2; 
    %-----------求解方程组-------------%
